...
class DataDumperLoader(object):
    ...
    def _process_example(self, smiles = "C1CCN(CC1)C(=O)C=CC=CC2=CC3=C(C=C2)OCO3"):
        ...
        eigVal_coulomb, eigVec_coulomb = self._cal_laplace_coulomb(coulomb_array["coulomb"])
...
class PyrfumeDumper(DataDumperLoader):
    ...
    def read_smiles_list(self):
        ...
        self.smiles_list = sorted([t.strip() for t in train_smiles + val_smiles + test_smiles])
        print("smiles: {}".format(len(self.smiles_list)))

if __name__ == '__main__':
    data_dir = "../pyrfume_sois_canon/dumped30_coulomb_frobenius/"
    atom2id_path = "../pyrfume_sois_canon/dumped30_coulomb_frobenius/atom2id.txt"
    dumper = PyrfumeDumper(data_dir, step_binary = -1, cou_norm = "frobenius", max_atoms = 62, max_size = 62, atom2id_path = atom2id_path, add_Hs = True, freq = 20)
    dumper.dump()
