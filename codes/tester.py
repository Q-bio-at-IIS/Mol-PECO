...
class Tester(object):
    ...
    def __init__(self, model, test_loader, labels, out_dir):
    ...
    def generate_emb_on_step(self, data):
        atom_feats, adj_feats, mol_masks = self._data_to_var(data)
        mol_embs = self.model.forward_emb(atom_feats, adj_feats, mol_masks)
        return mol_embs
...
class TesterCoulomb(object):
    ...
    def generate_emb_on_step(self, data):
        atom_feats, coulomb_feats, eigval_coulomb, eigvec_coulomb, mol_masks = self._data_to_var(data)
        mol_embs = self.model.forward_emb(atom_feats, coulomb_feats, mol_masks)
        return mol_embs
...
class TesterAdjCoulomb(object):
    """docstring for Tester"""
    def __init__(self, model, test_loader, labels, out_dir, method = "zscore"):
        super(TesterAdjCoulomb, self).__init__()
        self.model = model.cuda()
        self.criterion = nn.BCELoss()
        self.test_loader = test_loader
        self.labels_list = labels
    ...
    def _data_to_var(self, data):
        atom_feats, adj_feats, coulomb_feats = data["atom_feat"], data["adj_feat"], data["coulomb_feat"]
        eigval_coulomb, eigvec_coulomb = data["eigval_coulomb"], data["eigvec_coulomb"]
        mol_masks = data["mol_mask"]
        atom_feats = self._to_var(atom_feats, t = "int")
        adj_feats = self._to_var_dict(adj_feats)
        ...
        eigvec_coulomb = self._to_var(eigvec_coulomb, t = "float")
        mol_masks = self._to_var(mol_masks, t = "bool")
        return atom_feats, adj_feats, coulomb_feats, eigval_coulomb, eigvec_coulomb, mol_masks

    def test_on_step(self, data):
        atom_feats, adj_feats, coulomb_feats, eigval_coulomb, eigvec_coulomb, mol_masks = self._data_to_var(data)
        preds = self.model(atom_feats, adj_feats, coulomb_feats, mol_masks)
        return preds

    def generate_emb_on_step(self, data):
        atom_feats, adj_feats, coulomb_feats, eigval_coulomb, eigvec_coulomb, mol_masks = self._data_to_var(data)
        mol_embs = self.model.forward_emb(atom_feats, adj_feats, coulomb_feats, mol_masks)
        return mol_embs
